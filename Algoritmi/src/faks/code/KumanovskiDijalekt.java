...
public class KumanovskiDijalekt {
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i < pom.length; i++) {
            ...
            char last = zbor.charAt(zbor.length() - 1);
            boolean isUpper = Character.isUpperCase(zbor.charAt(0));

            SLLNode<MapEntry<String, String>> node = null;

            if (Character.isLetter(last)) {
                node = tabela.search(zbor.toLowerCase());
                if (node != null) {
                    zbor = node.element.value;
                    if (isUpper) {
                ...
                node = tabela.search(zbor.substring(0,zbor.length()-1).toLowerCase());
                if (node == null) {
                    System.out.print(zbor + " ");
                } else {
                    zbor = node.element.value;
