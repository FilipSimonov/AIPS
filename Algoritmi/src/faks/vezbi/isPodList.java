...
public class isPodList {
    private static boolean daliPodLista(DLL<Integer> list, DLL<Integer> temp, int tempSize) {
     DLLNode<Integer> node = list.getFirst();
     DLLNode<Integer> tmp = temp.getFirst();

     while (node!=null && tmp!=null){
         if (node.element == tmp.element){
             DLLNode<Integer> pos = node;

             while (pos!=null && tmp!=null && tempSize>0){
                 if (pos.element != tmp.element)
                     break;
                 pos = pos.succ;
                 tmp = tmp.succ;
                 --tempSize;
